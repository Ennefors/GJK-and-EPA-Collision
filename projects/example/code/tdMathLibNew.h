...
public:
	...
	{
		if (get_X() == compV.get_X() && get_Y() == compV.get_Y() && get_Z() == compV.get_Z()) {
			return true;
		}
		else {
			return false;
	...
	{
		fdVector temp;
		temp.set_Vector(get_X()*v, get_Y()*v, get_Z()*v);
		return temp;
	}
	//==opperator override
...
public:
	...
	void set_Rotation(fdVector v, float d)
	{
		...
		fdMat[0][0] = c + x2*(1-c);
		fdMat[0][1] = x1*y1*(1-c) - z1*s;
		fdMat[0][2] = x1*z1*(1-c) + y1*s;


		fdMat[1][0] = y1*x1*(1-c) + z1*s;
		fdMat[1][1] = c + y2*(1-c);
		fdMat[1][2] = y1*z1*(c-1) - x1*s;


		fdMat[2][0] = z1*x1*(1-c) - y1*s;
		fdMat[2][1] = z1*y1*(1-c) + x1*s;
		fdMat[2][2] = c + z2*(1-c);
    ...
    void otrhoNorm()
    {
        fdVector xax = fdVector(fdMat[0][0], fdMat[0][1], fdMat[0][2]);
        fdVector yax = fdVector(fdMat[1][0], fdMat[1][1], fdMat[1][2]);
        fdVector zax = fdVector(fdMat[2][0], fdMat[2][1], fdMat[2][2]);

        fdVector v1, v2, v3;

        v1 = xax.normalize();
        v2 = (yax-(v1*(yax*v1))).normalize();
        v3 = (zax-(v1*(zax*v1))).normalize();
        v3 = (v3-(v2*(v3*v2))).normalize();

        *this = fdMatrix(v1[0], v1[1], v1[2], 0,
                         v2[0], v2[1], v2[2], 0,
                         v3[0], v3[1], v3[2], 0,
                         0, 0, 0, 1);
    };
